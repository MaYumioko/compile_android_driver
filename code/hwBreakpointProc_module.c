...
static void hwbp_handler(struct perf_event *bp,
    ...
    for (iter = cvector_begin(g_hwbp_handle_info_arr); iter != cvector_end(g_hwbp_handle_info_arr); iter = cvector_next(g_hwbp_handle_info_arr, iter)) {
        ...
        // ↓ ↓ ↓ 新增的专属 Hook 检查 ↓ ↓ ↓
        // 1. 检查 *此断点* 是否绑定了 hook_pc
        if (hwbp_handle_info->hook_pc) {
            regs->pc = hwbp_handle_info->hook_pc; // 执行跳转
...
static ssize_t OnCmdInstProcessHwbp(struct ioctl_request *hdr, char __user* buf) {
	...
	uint64_t hook_pc = 0; // 默认为 0 (不跳转)
    // 检查 payload 缓冲区大小是否足够
    if (hdr->buf_size >= sizeof(uint64_t)) {
        // buf 指向的是 header 之后的 payload 区域
        if (x_copy_from_user(&hook_pc, (void __user*)buf, sizeof(uint64_t))) {
            printk_debug(KERN_INFO "x_copy_from_user hook_pc failed.\n");
            return -EFAULT;
        }
    }
	...
	printk_debug(KERN_INFO "hook_pc:%px\n", (void*)hook_pc);
	...
	hwbp_handle_info.hook_pc = hook_pc;
